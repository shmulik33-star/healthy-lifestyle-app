...
void main() {
  ...
  test('first equipment save creates a recovery copy', () async {
    const item = CustomEquipmentItem(
      id: 'backup-seed',
      name: 'משקולת לגיבוי',
      category: 'משקולות',
    ...
    expect(prefs.getString(EquipmentStore.backupStorageKey), isNotNull);
  });

  test('equipment recovers from backup and repairs corrupt primary data', () async {
    ...
    final prefs = await SharedPreferences.getInstance();
    expect(prefs.getString(EquipmentStore.storageKey), backup);
  });
